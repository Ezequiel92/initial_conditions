...
void check_error(const int err_type, const int err_code)
{
  ...
  terminate("Error code value = %d", err_code);
}
...
double eval_interp(struct InterpFunc *interp, const double x)
{
  if(isnan(interp->x_min) || isnan(interp->x_max))
    {
      terminate("x_min and/or x_max where not stored correctly");
    }
  ...
  while(*paths)
    {
      ...
      if(eta_table == NULL || cols != 3)
        {
          terminate("Something went wrong while trying to read the eta files");
        }
...
/*! \brief Compute the star formation rate according to our model.
 *
 *  This SFR can be used to calculate the probability of forming a star population.
 *
...
double rate_of_star_formation(const int index)
{
  double dt       = (P[index].TimeBinHydro ? (((integertime)1) << P[index].TimeBinHydro) : 0) * All.Timebase_interval;
  double int_time = dt * T_GYR / All.cf_hubble_a;

  double fa = 0.0, fi = 0.0, fm = 0.0;
  get_gas_fractions(index, &fi, &fa, &fm);
  ...
  if(globbuf.gl_pathc < 2)
    {
      terminate("Glob could not find all eta files");
    }
  ...
  for(int i = 0; i < DIVISIONS; ++i)
    {
      ...
      y[0] = fi;  /* Ionized fraction */
      y[1] = fa;  /* Atomic fraction */
      y[2] = fm;  /* Molecular fraction */
  ...
  SphP[index].ionizedFrac   = ionized_fraction;
  SphP[index].atomicFrac    = atomic_fraction;
  SphP[index].molecularFrac = molecular_fraction;
  SphP[index].metalFrac     = metal_fraction;
  SphP[index].stellarFrac   = star_fraction;

  /* Return the star formation rate in [Mₒ yr^(-1)] */
  return star_fraction * (P[index].Mass * M_COSMO) / (int_time * 1e9);
}
