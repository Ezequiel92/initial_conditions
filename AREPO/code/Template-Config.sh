...
#--------------------------------------- Magnetohydrodynamics; default: no magnetohydrodynamics
...
#USE_SFR                       # star formation model, turning dense gas into collisionless particles
#SFR_KEEP_CELLS                # do not destroy cell out of which a star has formed
...
#MULTIPLE_NODE_SOFTENING           # if a tree node is to be used which is softened, this is done with the softening of its different mass components
#INDIVIDUAL_GRAVITY_SOFTENING=2+4  # bitmask with particle types where the softening type should be chosen with that of parttype 1 as a reference type
#ADAPTIVE_HYDRO_SOFTENING          # adaptive softening of gas cells depending on their size
#NSOFTTYPES_HYDRO=64               # overrides number of discrete softening values for gas cells when ADAPTIVE_HYDRO_SOFTENING (default is 64)
...
#--------------------------------------- Static NFW Potential; default: no external potential
...
#--------------------------------------- Static Isothermal Sphere Potential; default: no external potential
...
#--------------------------------------- Static Hernquist Potential; default: no external potential
...
#--------------------------------------- MPI; default: default pinning by MPI
...
#--------------------------------------- On the fly FOF groupfinder; default: no friends-of-friends group finder
...
#--------------------------------------- Subfind; default: no Subfind subtructure finder
...
#--------------------------------------- Output fields in snapshots--Default output fields are: position, velocity, ID, mass, spec. internal energy (gas), density(gas)
...
#OUTPUT_VERTEX_VELOCITY_DIVERGENCE # output divergence of cell velocity
#OUTPUT_VOLUME                 # output volume of cells; note that this can always be computed as both, density and mass of cells are by default in output
...
#OUTPUT_SOFTENINGS             # output particle softening
...
#--------------------------------------- Output options
#PROCESS_TIMES_OF_OUTPUTLIST   # goes through times of output list prior to starting the simulation to ensure that outputs are written as close to the desired time as possible (as opposed to at next possible time if this flag is not active)
#REDUCE_FLUSH                  # only flush output to log-files in predefined intervals
#OUTPUT_EVERY_STEP             # create snapshot on every (global) synchronization point, independent of parameters chosen or output list.
